...
const recipeSchema = Joi.object({
  ...
  name: Joi.string()
    ...
    .required(),
  imgUrl: Joi.string().trim().uri().message('Recipe image must be a valid url'),
  ingredients: Joi.array().items(ingredientSchema).required(),
  steps: Joi.array().items(
    Joi.string()
      .trim()
      .min(3)
      .message('Cooking step must be at least 3 characters long')
  ),
  prepTime: Joi.number()
    .positive()
    .min(1)
    .message('Cooking time must be more than 1 second'),
  portions: Joi.number()
    .positive()
    .min(1)
    .message('Meal cannot be less than 1 portion'),
